    ...
    export default {
        ...
        created() {
            ...
            db.collection('services').orderBy('date').onSnapshot((snapshotChange) => {
                this.Services = [];
                snapshotChange.forEach((doc) => {
                    console.log("AAAAAAAAAAAAAAAA", new Date(doc.data().date).getTime())
                    if(doc.data().pending !== true && new Date(doc.data().date).getTime() <= new Date(new Date().setHours(0, 0, 0, 0)).getTime()) {
                    this.FullServices.splice(0, 0, {
                        ...
                        cem: this.formatCL(doc.data().cem, doc.data().cemnotes, doc.data().casket, doc.data().cremation),
